    ...
    <div className="h-[calc(100vh-80px)] overflow-hidden">
      ...
      >
        {slides.map((slide) => (
          ...
          >
            {/* TEXT CONTAINER  */}
            <div className="h-1/2 xl:w-1/2 xl:h-full flex flex-col justify-center gap-8 items-center 2xl:gap-12 text-center">
              <h2 className="text-xl lg:text-3xl 2xl:text-5xl">
                {slide.description}
              </h2>
              <h1 className="text-5xl md:text-4xl lg:text-6xl 2xl:text-8xl font-semibold">
                {slide.title}
              </h1>
              <Link href={slide.url}>
      ...
      <div className="absolute m-auto left-1/2 bottom-8 flex gap-4">
        {slides.map((slide, index) => (
          <div
            className={`w-3 h-3 rounded-full ring-1 ring-gray-300 cursor-pointer flex items-center justify-center ${
